...
export default async function ProjectsPage({ searchParams }: PageProps) {
  ...
  if (sort === 'cost') {
    ...
    const allIds = (await db.project.findMany({ where: baseWhere, select: { id: true } })).map(p => p.id)
    total = allIds.length
              ...
              {projects.map((project) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 font-mono text-muted-foreground hidden sm:table-cell">
                      {projectCost > 0 ? `€${projectCost.toFixed(2)}` : <span className="text-muted-foreground/40">—</span>}
                    </td>
                    <td className="px-3 py-2.5">
