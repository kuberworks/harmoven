...
`
    ...
    // relDir: path prefix relative to /home/pyodide (empty string at root, e.g. "src/main/java")
    function collectFiles(py, dir, relDir) {
      relDir = relDir || ''
      const result = []
      let totalBytes = 0
      try {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            if (py.FS.isDir(stat.mode)) {
              result.push(...collectFiles(py, fullPath, relPath))
              continue
            }
