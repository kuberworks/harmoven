...
// Print CSS moved to lib/utils/print-css.ts — loaded lazily inside handlePrint().
...
function ResultTab({
  ...
  async function handlePrint() {
    ...
    // Open the window synchronously (before any await) so popup blockers see
    // it as a direct response to the user's click event.
    ...
    // Lazy-load the print CSS now that we have a window handle.
    // This module is excluded from the initial bundle — loaded only on first print.
    const { PRINT_CSS } = await import('@/lib/utils/print-css')
...
<tbody><tr><td>${sectionsHtml}</td></tr></tbody>
    ...
    if (blobUrl) {
      // Navigate the already-opened empty window to the blob URL.
      // onload fires for the blob navigation, not the initial empty URL.
