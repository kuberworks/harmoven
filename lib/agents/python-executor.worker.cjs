...
;(async () => {
  ...
  try {
    ...
    if (Array.isArray(workerData.packages) && workerData.packages.length > 0) {
      ...
      // so that their transitive stdlib dependencies (e.g. pandas → os) are already
      // in sys.modules BEFORE the security firewall is installed.
      // Without pre-import, the first `import pandas` in user code runs
      // pandas/__init__.py which does `import os` → blocked by the firewall.
      const pkgJson = JSON.stringify(workerData.packages)
      await py.runPythonAsync(`
import micropip as _mp
await _mp.install(${pkgJson})
# Pre-import to pull transitive stdlib deps into sys.modules before the firewall.
for _pkg in ${pkgJson}:
...
`)
      ...
      // Auto-detect mode: parse imports from the AST, install, then pre-import.
      // Pre-importing is the key step: it ensures that when the security firewall
      // is installed (next block), packages like pandas, numpy, matplotlib etc.
      // are already fully loaded and their internal `import os` / `import sys`
      // calls have already completed. The firewall never sees those transitive
      // imports, so it can safely block os/socket/... for NEW imports in user code.
...
async def _auto_install():
    ...
    # Pre-import ALL detected non-stdlib names (installed or not) so their
    # transitive dependencies land in sys.modules before the firewall.
    for _name in _names:
...
`
    ...
    // At this point all third-party packages and their stdlib deps are already
    // in sys.modules, so the firewall only blocks NEW imports of blocked modules.
