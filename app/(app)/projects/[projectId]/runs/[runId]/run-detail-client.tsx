...
function ResultTab({
  ...
  // Fetch all artifacts generated by PYTHON_EXECUTOR nodes in this run.
  // PYTHON_EXECUTOR is the only agent that produces downloadable binary files
  // (Excel, CSV, images, PDFs, etc.) — text/code WRITER outputs are rendered inline.
  const [runArtifacts, setRunArtifacts] = useState<ArtifactMeta[]>([])
  useEffect(() => {
    const hasPython = nodes.some(
      n => n.agent_type === 'PYTHON_EXECUTOR' && n.status === 'COMPLETED',
    )
    if (!hasPython) return
    fetch(`/api/runs/${runId}/artifacts`)
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
