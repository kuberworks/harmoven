...
;(async () => {
  ...
  try {
    ...
    // Install third-party packages via micropip (PyPI) if requested.
    // Package download goes through Node.js fetch (Emscripten bridge), NOT Python sockets,
    // so this step must happen BEFORE the import firewall is installed.
    if (Array.isArray(workerData.packages) && workerData.packages.length > 0) {
      await py.loadPackage('micropip')
      // runPythonAsync supports top-level `await` — required for micropip.install.
