...
;(async () => {
  ...
  try {
    // require('pyodide') resolves via standard Node.js module resolution from this
    // .cjs file's location — no path needed from the parent. The parent formerly
    // passed pyodidePath, but require.resolve() in the Next.js/webpack build is
    // replaced with a numeric module ID which makes require(number) throw.
...
`)
    ...
    // micropip is pre-downloaded in the Docker image (Dockerfile deps stage) to avoid
    // CDN fetches at runtime and EACCES errors when Pyodide tries to cache the wheel
    // into node_modules (which is root-owned in the container). Wrap in try/catch so
    // the worker does not crash if the wheel is somehow missing — user code will just
    // get ImportError for any packages it needs.
    try {
      await py.loadPackage('micropip')
    } catch (_) {
      // micropip unavailable — skip all auto/explicit package installation
    }
    ...
    if (Array.isArray(workerData.packages) && workerData.packages.length > 0) {
      ...
      // Install errors are swallowed (matching auto-detect behaviour) so the worker
      // does not crash. Packages unavailable in Pyodide's WASM repo or on PyPI as
      // pure-Python wheels will surface as a normal ImportError in user code.
...
async def _auto_install():
    ...
    try:
        ...
        return  # micropip not available — skip; user code gets ImportError for missing pkgs
    _code = ${JSON.stringify(workerData.code)}
    _names = set()
    try:
        _tree = ast.parse(_code)
    except SyntaxError:
        _tree = None  # code is syntactically invalid; skip auto-install, let runPython surface the error
...
`
    ...
    // Defined here so _effectiveAllowed is available both for the
    // SUPPORTED_EXTENSIONS injection below AND for collectFiles later.
    ...
    const ALLOWED_MIME = {
      ...
      'jsonl':   'text/plain',              // newline-delimited JSON
      'ndjson':  'text/plain',              // same spec, alternate extension
      ...
      'rtf':     'application/rtf',
      // Document / markup / documentation
      'txt':     'text/plain',
      'md':      'text/markdown',
      'rst':     'text/plain',              // reStructuredText
      'adoc':    'text/plain',              // AsciiDoc
      'tex':     'text/plain',              // LaTeX source
      'html':    'text/html',              // kept for preview potential — download-only on client
      'htm':     'text/html',
      'xml':     'text/xml',
      'xsd':     'text/xml',               // XML Schema
      'css':     'text/css',
      'sql':     'text/plain',
      'ipynb':   'application/json',        // Jupyter notebook
      // Images
      'png':     'image/png',
      'jpg':     'image/jpeg',
      'jpeg':    'image/jpeg',
      ...
      'ttf':     'font/ttf',
      'otf':     'font/otf',
      // CSS preprocessors / web templates
      'scss':    'text/plain',
      'sass':    'text/plain',
      'less':    'text/plain',
      'njk':     'text/plain',              // Nunjucks
      'hbs':     'text/plain',              // Handlebars
      'ejs':     'text/plain',              // EJS
      'pug':     'text/plain',              // Pug/Jade
      ...
      'py':      'text/plain',
      'js':      'text/plain',
      'mjs':     'text/plain',              // ES module
      'cjs':     'text/plain',              // CommonJS module
      'ts':      'text/plain',
      'mts':     'text/plain',              // TypeScript ES module
      'jsx':     'text/plain',
      'tsx':     'text/plain',
      'vue':     'text/plain',              // Vue Single File Component
      'svelte':  'text/plain',              // Svelte component
      'astro':   'text/plain',              // Astro component
      'elm':     'text/plain',              // Elm
      'go':      'text/plain',
      'rs':      'text/plain',
      'zig':     'text/plain',              // Zig
      'nim':     'text/plain',              // Nim
      'dart':    'text/plain',              // Dart / Flutter
      'c':       'text/plain',
      'cpp':     'text/plain',
      'h':       'text/plain',
      'hpp':     'text/plain',
      ...
      'jl':      'text/plain',              // Julia
      'pl':      'text/plain',              // Perl
      'pm':      'text/plain',              // Perl module
      'ex':      'text/plain',              // Elixir
      'exs':     'text/plain',              // Elixir script
      'erl':     'text/plain',              // Erlang
      'hs':      'text/plain',              // Haskell
      'ml':      'text/plain',              // OCaml / Standard ML
      'mli':     'text/plain',              // OCaml interface
      'clj':     'text/plain',              // Clojure
      // Schema / API definition
      'proto':   'text/plain',              // Protocol Buffers
      'graphql': 'text/plain',              // GraphQL schema
      'gql':     'text/plain',              // GraphQL (alt extension)
      // Build / config files
      ...
      'gradle':  'text/plain',
      'kts':     'text/plain',              // Kotlin Gradle build scripts
      'cmake':   'text/plain',              // CMake build script
      'bzl':     'text/plain',              // Bazel build rule
      'tf':      'text/plain',              // Terraform HCL
      'hcl':     'text/plain',              // HashiCorp Configuration Language
      'nix':     'text/plain',              // Nix expression
      'ini':     'text/plain',
      'conf':    'text/plain',
      'cfg':     'text/plain',
      'plist':   'text/xml',               // Apple property list (XML)
      'xcconfig':'text/plain',             // Xcode build configuration
      'strings': 'text/plain',             // iOS / macOS localizable strings
      'sh':      'text/plain',
      'bat':     'text/plain',
      'env':     'text/plain',
      'lock':    'text/plain',              // package-lock, Cargo.lock, poetry.lock, etc.
      'gitignore':   'text/plain',
      'editorconfig':'text/plain',
      'dockerfile':  'text/plain',
      'makefile':    'text/plain',
      'mk':      'text/plain',
      'pro':     'text/plain',              // ProGuard/R8 rules (Android)
      'feature': 'text/plain',              // Cucumber / Gherkin BDD scenarios
    }
    const MAX_FILE_BYTES    = 10 * 1024 * 1024   // 10 MB per file
    const MAX_TOTAL_BYTES   = 50 * 1024 * 1024   // 50 MB total per run

    // Build effective allowed map from per-node overrides.
    // allowedExtensions (non-null) → restrict to only those keys (must exist in ALLOWED_MIME).
    // blockedExtensions            → always removed from the result.
    const _rawAllowed = Array.isArray(workerData.allowedExtensions) && workerData.allowedExtensions.length > 0
      ? workerData.allowedExtensions.reduce((acc, ext) => {
          const key = String(ext).toLowerCase()
          if (ALLOWED_MIME[key]) acc[key] = ALLOWED_MIME[key]
          return acc
        }, {})
      : { ...ALLOWED_MIME }
    const _blocked = new Set((workerData.blockedExtensions ?? []).map(e => String(e).toLowerCase()))
    const _effectiveAllowed = Object.fromEntries(
      Object.entries(_rawAllowed).filter(([k]) => !_blocked.has(k))
    )
    ...
    // Inject SUPPORTED_EXTENSIONS frozenset so generated code can introspect at runtime.
    const _exts = JSON.stringify(Object.keys(_effectiveAllowed))
    py.runPython(`SUPPORTED_EXTENSIONS = frozenset(${_exts})`)

    py.runPython(workerData.code)

    // ─── Auto-extract ZIPs ────────────────────────────────────────────────────
    // If user code created any .zip files in /home/pyodide, extract them in-place
    // before collectFiles runs. This lets the LLM use zipfile for complex directory
    // trees (e.g. Spring Boot with src/main/java/... hierarchy) while the platform
    // still collects every individual file — including README.md — and the REVIEWER
    // can inspect the content. The .zip itself is removed after extraction so it
    // does not appear as an unknown_ext skipped file.
    //
    // Extraction is best-effort: if a ZIP is malformed or extraction fails the .zip
    // stays in the VFS and is simply skipped by collectFiles (unknown_ext).
    py.runPython(`
import zipfile as _zf, os as _ose, pathlib as _pl
_home = '/home/pyodide'
for _fname in list(_ose.listdir(_home)):
    if not _fname.endswith('.zip'):
        continue
    _zpath = _ose.path.join(_home, _fname)
    try:
        with _zf.ZipFile(_zpath, 'r') as _z:
            _z.extractall(_home)
        _ose.remove(_zpath)
    except Exception:
        pass  # leave as-is; collectFiles will skip it as unknown_ext
del _zf, _ose, _pl, _home
`)

    // ─── Collect generated files ──────────────────────────────────────────────
    // Scan /home/pyodide after execution. Transfer raw ArrayBuffers (zero-copy).
    // Files outside _effectiveAllowed or over MAX_FILE_BYTES are silently skipped.
    // This must run AFTER user code so we only collect intentionally generated files.
    // ALLOWED_MIME, MAX_FILE_BYTES, _effectiveAllowed, SILENT_SKIP_EXTS are defined
    // above (before user code) so SUPPORTED_EXTENSIONS can be injected at runtime.
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            const mime = _effectiveAllowed[ext]
            if (!mime) {
